...
namespace Voron
{
    public unsafe class Slice
    {
        ...
        public override string ToString()
        {
            ...
            if (_array != null)
            {
                if (char.IsLetterOrDigit((char) _array[0]))
                {
            ...
            if (char.IsLetterOrDigit((char) *_pointer))
            {
                return new string((sbyte*) _pointer, 0, _pointerSize, Encoding.UTF8);
            }

            var hex = new StringBuilder(_pointerSize * 2);
            for (int i = 0; i < _pointerSize; i++)
            {
                hex.AppendFormat("{0:x2}-", *(_pointer + i));
            }
            return hex.ToString();
        }
        ...
        public void CopyTo(byte[] dest)
        {
            if (_array == null)
            {
                if(dest.Length < _pointerSize)
                    throw new ArgumentException("dest is too small");
                fixed (byte* a = dest)
                {
                    NativeMethods.memcpy(a, _pointer, _pointerSize);
                }
                return;
            }
